function renderPlans() {
  ...
  container.innerHTML = siteConfig.plans
    .map((plan) => {
      ...
      const featured = plan.featured ? " plan-card--featured" : "";
      return `
        <div class="plan-card${featured}">
          ...
          <p class="plan-details">${lang.details}</p>
          <a href="${whatsappUrl(lang.message)}" class="btn btn-whatsapp btn-block" target="_blank" rel="noopener">
...
function renderMenu() {
  ...
  container.innerHTML = siteConfig.menu
    .map(
      (item) => `
      <div class="menu-card">
        <span class="menu-day">${item[currentLang].day}</span>
        <p class="menu-items">${item[currentLang].items}</p>
      </div>`
    )
    .join("");
}
